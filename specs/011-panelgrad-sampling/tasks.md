...
- [ ] T034 [P] Add explicit regressions proving non-PanelGrad checkpoints contain no PanelGrad state and Thompson, UCB, random global/per-block, nested-all, and standalone semantics remain unchanged in `tests/test_artifacts.py`, `tests/test_probabilistic_controller.py`, and `tests/test_adaptive_sampler.py`
- [ ] T035 [P] Verify the PanelGrad preflight and short smoke commands, and record any corrected commands or expected artifact checks in `specs/011-panelgrad-sampling/quickstart.md`
