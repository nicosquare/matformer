...
**Configuration Inputs**: Existing YAML and dotted overrides; `adaptive_global`, `panelgrad`, resolved granularities, optional balanced warmup, and `model.panelgrad.{refresh_interval_steps,eta,temperature,epsilon|epsilon_schedule}`
...
**Reproducibility Notes**: Existing strict runtime and role seeds; new independent `panelgrad_sampling` stream; rank-zero categorical authority; complete refresh phase, active epsilon/schedule step, and RNG state persisted; same-topology comparisons use `rtol=1e-6`, `atol=1e-8` for numeric state and exact discrete actions
