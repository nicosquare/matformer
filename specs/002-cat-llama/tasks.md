...
**Purpose**: Make distributed training schedule and optimizer behavior config-driven so `cat_llama` and `matformer_llama` runs remain comparable while FSDP debugging can switch optimizer settings explicitly.

**Independent Test**: Resolve a config with `training.learning_rate_scale_rule`, `training.warmup_ratio` or `training.warmup_steps`, and `training.optimizer.{name,kwargs}`, then confirm the resolved config and run summary persist the effective learning rate, warmup policy, and optimizer metadata.
...
- [X] T015 Update `training/run.py` and `train.py` to build the optimizer from the resolved config, apply the resolved warmup schedule from `training.max_steps`, and keep `adamw` as the default while allowing `sgd` for debug runs
- [X] T016 Extend `utils/metrics.py` and the artifact-writing helpers used by `training/run.py` so `config.json` and `run_summary.json` persist `base_learning_rate`, `learning_rate_scale_rule`, `learning_rate_scale_factor`, `resolved_learning_rate`, `warmup_ratio`, `warmup_steps`, `resolved_warmup_steps`, `optimizer_name`, and `optimizer_kwargs`
- [X] T017 Add focused smoke coverage in `tests/test_config.py`, `tests/test_training_smoke.py`, and `tests/test_artifacts.py` for learning-rate scaling, warmup precedence, optimizer switching, and resolved artifact metadata

**Checkpoint**: Phase 6.1 should now be independently verifiable with config, training, and artifact smoke checks.
