...
def legacy_state(rows):
    """Build the historical schema explicitly; current writers are bounded."""
    rows = list(rows)
    accumulator = StreamingMetricsAccumulator()
    accumulator.update(rows)
    state = accumulator.state_dict()
    state.pop('optimizer_last_attempt_key')
    state['schema_version'] = 1
    state['optimizer_attempt_ids'] = sorted({
        item['optimizer_action_id'].split('|optimizer_step=')[0] for item in rows
    })
    return state
...
def test_ordered_accounting_matches_legacy_with_duplicate_and_failed_rows():
    ...
    for key in old.keys() - {'schema_version', 'optimizer_last_attempt_key'}:
...
def test_campaign_bookkeeping_and_rollback_do_not_retain_history():
    ...
    assert not hasattr(compact, "optimizer_attempt_ids")
...
def test_legacy_history_is_converted_once_to_a_compact_marker(count):
    state = legacy_state(row(i, width=['g250', 'g500', 'g750', 'g1000'][i % 4]) for i in range(count))
...
def test_migration_rejects_incomplete_or_ambiguous_legacy_history(damage):
    state = legacy_state(row(i) for i in range(3))
...
def train_with_journal(bundle, *, stop=None, legacy=False):
    ...
    if legacy:
        import csv
        journal.flush()
        with open(config['run']['output_dir'] + '/metrics.csv') as stream:
            rows = [item for item in csv.DictReader(stream) if item['split'] == 'train']
        metrics = dict(state['metrics_accumulator_state'])
        metrics.pop('optimizer_last_attempt_id')
        metrics['schema_version'] = 1
        metrics['optimizer_attempt_ids'] = sorted({
            item['optimizer_action_id'].split('|optimizer_step=')[0] for item in rows
        })
        state['metrics_accumulator_state'] = metrics
    return journal
...
def test_step_scoped_campaign_ids_preserve_historical_marker_on_resume():
    compact = StreamingMetricsAccumulator(ordered_attempts=True)
    first = row(0)
    first['optimizer_action_id'] += '|optimizer_step=1'
    compact.update([first])
    restored = StreamingMetricsAccumulator(compact.state_dict())
    restored.update([first, row(1)])
    assert restored.attempted_optimizer_steps == 2
    assert restored.optimizer_last_attempt_id == row(1)['optimizer_action_id']
    before = copy.deepcopy(restored.state_dict())
    invalid = row(2)
    invalid['optimizer_action_id'] += '|optimizer_step=4'
