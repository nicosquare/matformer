"""Admission refuses ambiguous/global limits and stale validation evidence."""
import importlib.util
from pathlib import Path
import pytest

spec=importlib.util.spec_from_file_location('im_queue',Path('scripts/run_tinystories_inverse_membership.py'))
queue=importlib.util.module_from_spec(spec)
spec.loader.exec_module(queue)


def test_exact_five_arms_and_root():
    assert queue.ARMS == ('S1-IM','S2-IM','C1-IM','C2-IM','C3-IM')
    assert queue.BASE.name == 'inverse-membership-sampling-v1'


@pytest.mark.parametrize('running,submitted,expected',[(2,4,(2,4)),(1,2,(1,2)),(3,4,None),(2,5,None),(0,4,None)])
def test_limits_accept_stricter_only(monkeypatch,running,submitted,expected):
    text=f'QOS={queue.QOS}(1)\n      ivo.navarrete(123)\n        MaxJobsPU={running}(0) MaxSubmitJobsPU={submitted}(0)\n'
    monkeypatch.setattr(queue,'command',lambda args: 'AccountingStorageEnforce = limits,qos' if args[-1]=='config' else text)
    if expected is None:
        with pytest.raises(RuntimeError): queue.verify_scheduler_limits()
    else:
        limits=queue.verify_scheduler_limits()
        assert (limits['max_running'],limits['max_submitted'])==expected


def test_worker_requires_slurm(monkeypatch):
    monkeypatch.delenv('SLURM_JOB_ID',raising=False)
    with pytest.raises(RuntimeError,match='sbatch'): queue.worker('S1-IM',{})
...
from pathlib import Path
import pytest
from scripts import run_tinystories_inverse_membership as queue
...
def test_queue_submits_four_and_restart_does_not_duplicate(setup, monkeypatch):
    ...
    record = json.loads((root/'launchers/submissions.json').read_text())
    assert record['jobs'][-1]['arm_id'] == 'C3-IM'
...
def test_restart_reconciles_uncertain_submission_by_unique_name(setup, monkeypatch):
    root, plan = setup
    name = 'oo-im-v1-S1-IM-a1'
    (root/'launchers/submissions.json').write_text(json.dumps({'jobs':[{'arm_id':'S1-IM','name':name,'status':'submitting','gpu_index':0}]}))
...
def test_gpu_monitor_selects_allocated_device_not_last_inventory_row(setup):
    root, plan = setup
    (root/'logs/S1-IM-10-gpu.csv').write_text('timestamp, uuid, utilization.gpu, memory.used\nnow, GPU-a, 20 %, 1000 MiB\nnow, GPU-b, 99 %, 30000 MiB\n')
    assert 'GPU-a' in queue.progress('S1-IM', 0)['gpu_observation']
    assert queue.progress('S1-IM')['gpu_observation'] is None
    (root/'logs/S1-IM-10-gpu.csv').write_text('index, timestamp, uuid, utilization.gpu, memory.used\n0, now, GPU-a, 20 %, 1000 MiB\n1, now, GPU-b, 99 %, 30000 MiB\n')
    assert 'GPU-a' in queue.progress('S1-IM', 0)['gpu_observation']
