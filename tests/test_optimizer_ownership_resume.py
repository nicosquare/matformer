...
def test_campaign_restore_installs_validated_sign_dynamics_state(tmp_path, monkeypatch):
    bundle = packed_fixture(tmp_path)
    train(bundle, stop=1)
    path = tmp_path / 'with-sign-dynamics.pt'
    save(bundle, path)
    target = packed_fixture(tmp_path)
    config, model, optimizer, scheduler, batches, _ = target
    validated = {'last_completed_step': 1}

    class SignRuntime:
        restored = None

        def validate_checkpoint_state(self, state, *, expected_step):
            assert expected_step == 1
            return validated

        def restore_validated_state(self, state):
            assert state is validated
            self.restored = state

        def state_dict(self, *, copy_tensors):
            assert copy_tensors is False
            return self.restored

    runtime = SignRuntime()
    monkeypatch.setattr(cp, 'uses_sign_dynamics', lambda config: True)
    restored = cp.load_checkpoint_state(
        path, model, optimizer, scheduler, config=config,
        train_dataloader=batches, sign_dynamics_runtime=runtime,
    )
    assert restored['sign_dynamics_state'] is validated
    assert_state_equal(model.state_dict(), bundle[1].state_dict())
