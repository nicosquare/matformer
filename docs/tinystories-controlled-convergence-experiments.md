# TinyStories recipe selection and elastic experiments
...
After selection, create the profile-specific path printed as `$CAPACITY_CONFIG`
(`configs/controlled_exps/tinystories_instruct_capacity_converged.yaml` for
`instruct`) rather than changing `tinystories_controlled_convergence.yaml`,
which is the immutable ordinary-TinyStories d128 baseline. Record at least the selected model shape,
